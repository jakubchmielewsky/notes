...
const Setting: React.FC = () => {
    ...
    const [selectedSettings, setSelectedSettings] = useState({theme: theme, font:font});
    const auth = getAuth();
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if(validate()){
            try {
                ...
                if(user && user.email) {
                    //re-authentication
                    const credential = EmailAuthProvider.credential(user.email, formData.old)
                ...
                <div className="flex flex-col gap-250 text-custom-neutral-950 dark:text-white">
                    ...
                    <ul className="flex flex-col gap-200">
                        {options.map((option, index) => (
                            <li key={index}>
                                <button onClick={()=>handleSelectSetting(option.title)} className={`flex items-center gap-200 py-200 px-200 border w-full rounded-12 dark:border-custom-neutral-800 ${(option.title === selectedSettings.theme || option.title === selectedSettings.font)&&"bg-custom-neutral-100 dark:bg-custom-neutral-800"}`}>
                                    <div className="w-full flex justify-between items-center">
                                        ...
                                        <div className={`border-2 w-200 h-200 rounded-full ${(option.title === selectedSettings.theme || option.title === selectedSettings.font)?"border-4 border-custom-blue-500 dark:border-custom-blue-500":"border-custom-neutral-200 dark:border-custom-neutral-600"}`}>
