...
const TagsDesktop: React.FC = () => {
    ...
    const filteredNotes =
        notes?.filter((note) => {
            if (
                note.tags?.some((tag) =>
                    tag.toLowerCase().includes(params.tagName || "")
                )
            ) {
                return true;
            }
    ...
    return (
        <div className="h-full w-full flex overflow-y-auto">
            <div
                className={`relative h-full max-h-full w-[290px] flex flex-col px-200 pt-250 gap-200 border-r-1 border-custom-neutral-200 dark:border-custom-neutral-800 dark:bg-custom-neutral-950 dark:text-white tablet:px-400 tablet:py-300`}
            >
                <button
